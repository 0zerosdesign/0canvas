...
//
//   import { Zeros } from "@Withso/zeros";
//   <Zeros />
//
...
//   - All UI is marked with data-Zeros so the inspector skips it
//   - CSS is injected at runtime via zeros-styles.ts
...
import { injectStyles, removeStyles } from "./zeros-styles";
...
export interface ZerosProps {
  ...
  /** Optional callback when Zeros opens/closes */
...
const POSITION_STYLES: Record<
  NonNullable<ZerosProps["position"]>,
  React.CSSProperties
> = {
...
export function AutoConnect({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!state.project) {
      ...
      // Zeros itself, not a user project — using it as the preview src
...
function ToggleButton({
  ...
  position: NonNullable<ZerosProps["position"]>;
  zIndex: number;
  shortcut: string;
  onClick: () => void;
}) {
  return (
    <Button
      ...
      size="icon"
      onClick={onClick}
      title={`Open Zeros (Ctrl+Shift+${shortcut.toUpperCase()})`}
      data-Zeros="toggle"
...
// ── Main Zeros Component ──────────────────────────────

const IFRAME_GUARD =
  typeof window !== "undefined" && window.name === "Zeros-preview";
...
export function Zeros({
  ...
  useEffect(() => {
    ...
    container.id = "Zeros-portal";
    container.setAttribute("data-Zeros", "portal");
...
export function EngineWorkspace({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <div className="oc-app-shell" data-Zeros="workspace">
      {/* Far-left sidebar */}
