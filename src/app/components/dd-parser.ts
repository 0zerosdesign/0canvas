// ──────────────────────────────────────────────────────────
// DD Parser — Convert between HTML/CSS and .dd JSON format
// ──────────────────────────────────────────────────────────
//
// Two directions:
//   1. htmlToDDTree()  — parse HTML string + computed styles into DDNode[]
//   2. ddTreeToHtml()  — render DDNode[] back to HTML + CSS strings
//
// ──────────────────────────────────────────────────────────
...
const STYLE_PROPS: (keyof DDStyles)[] = [
...
/**
 * Parse an HTML string + CSS into a DDDocument.
 * Uses a temporary detached document to parse.
 */
export function htmlToDDDocument(
  html: string,
  css: string,
  name: string,
  ...
  try {
    ...
    const variables: Record<string, DDVariable> = {};
...
function resolveValue(val: string | number, variables?: Record<string, DDVariable>): string {
...
function renderNode(node: DDNode, variables?: Record<string, DDVariable>): string {
  const tag = node.tag;
  const attrs: string[] = [];

  if (node.id) attrs.push(`data-dd-id="${node.id}"`);
...
function generateResponsiveCSS(
  tree: DDNode[],
  breakpoints: DDDocument["breakpoints"],
  variables?: Record<string, DDVariable>,
): string {
  ...
  function collectResponsive(nodes: DDNode[]) {
    for (const node of nodes) {
      if (node.responsive) {
        for (const [bp, overrides] of Object.entries(node.responsive)) {
          ...
          const cssProps = stylesToCSSString(overrides as DDStyles, variables);
          if (cssProps) {
            mediaRules[mq].push(`[data-dd-id="${node.id}"] { ${cssProps}; }`);
          }
        }
      }
...
/**
 * Render a DDDocument to { html, css } strings for preview.
 */
export function ddDocumentToHtml(doc: DDDocument): { html: string; css: string } {
