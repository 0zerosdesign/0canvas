// ──────────────────────────────────────────────────────────
// DD Project — Whole-project .0c file format
// ──────────────────────────────────────────────────────────
//
// A single .0c file is the canonical representation of an
// entire ZeroCanvas project: metadata, workspace config,
...
const LayerNodeSchema: z.ZodType<OCLayerNode> = z.lazy(() =>
  z.object({
    id: idString,
    tag: nonEmptyString,
...
export const OCProjectFileSchema = z.object({
  $schema: z.string().url().optional(),
  schemaVersion: z.literal(OC_PROJECT_SCHEMA_VERSION),
...
export type OCProjectFile = z.infer<typeof OCProjectFileSchema>;
export type OCProjectMeta = z.infer<typeof ProjectMetaSchema>;
export type OCWorkspaceMeta = z.infer<typeof WorkspaceMetaSchema>;
export type OCBreakpointsConfig = z.infer<typeof BreakpointsSchema>;
export type OCPage = z.infer<typeof PageSchema>;
export type OCVariant = z.infer<typeof VariantSchema>;
export type OCAnnotation = z.infer<typeof AnnotationSchema>;
...
export type OCLayerNode = {
  ...
  children: OCLayerNode[];
};

// ── Default breakpoints ────────────────────────────────────

export const DEFAULT_PROJECT_BREAKPOINTS: OCBreakpointsConfig = {
  desktop: 1280,
  laptop: 1024,
  tablet: 768,
  mobile: 390,
...
export type OCValidationResult =
  | { valid: true; data: OCProjectFile }
  | { valid: false; errors: string[] };

export function validateOCProjectFile(input: unknown): OCValidationResult {
  const result = OCProjectFileSchema.safeParse(input);
  if (result.success) {
...
export function migrateProjectFile(raw: any): OCProjectFile | null {
  ...
  if (version > OC_PROJECT_SCHEMA_VERSION) return null;

  // Run sequential migrations
  for (let v = version; v < OC_PROJECT_SCHEMA_VERSION; v++) {
...
export async function computeProjectHash(doc: OCProjectFile): Promise<string> {
...
export function createEmptyProjectFile(
  project: OCProject,
  framework: OCWorkspaceMeta["framework"] = "unknown",
): OCProjectFile {
  const now = new Date().toISOString();
  return {
    $schema: "https://zeros.design/schemas/oc-project-v1.json",
    schemaVersion: OC_PROJECT_SCHEMA_VERSION,
    project: {
      id: project.id,
      name: project.name,
      createdAt: now,
    ...
    integrity: {
      hash: "",
      generator: `0canvas@${typeof __VERSION__ !== "undefined" ? __VERSION__ : "0.0.0"}`,
    },
  };
}

// ── Convert runtime state → .0c project file ──────────────
...
// ── Convert .0c project file → runtime state ──────────────

export function projectFileToState(file: OCProjectFile): {
  project: OCProject;
  variants: VariantData[];
  feedbackItems: FeedbackItem[];
} {
...
export function serializeProjectFile(file: OCProjectFile): string {
  return JSON.stringify(file, null, 2);
}

// ── Parse from JSON string ─────────────────────────────────

export function parseProjectFile(json: string): OCValidationResult {
  try {
    const raw = JSON.parse(json);
    // Try migration first
    if (raw.schemaVersion && raw.schemaVersion < OC_PROJECT_SCHEMA_VERSION) {
    ...
    return validateOCProjectFile(raw);
  } catch (err) {
