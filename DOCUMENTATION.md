# DesignDead — Complete Project Documentation
...
2. [Module Reference](#2-module-reference)
   - [DesignDead Engine](#21-designdead-engine)
...
4. [.dd Format System](#4-dd-format-system)
   - [Variant .dd Format](#41-variant-dd-format)
   - [Project .dd File](#42-project-dd-file)
...
DesignDead is a **zero-dependency browser overlay** that wraps around any web application. It works by:

1. **Portal rendering** — The `<DesignDead />` React component creates a portal on `document.body` with z-index `2147483640`, placing its entire UI above the consumer's app.

2. **Iframe isolation** — The consumer's app is loaded inside an `<iframe name="designdead-preview">` within the Source Node. The `<DesignDead />` component detects when it's running inside this iframe and returns `null` (via the `IFRAME_GUARD`), preventing recursive rendering.
...
src/
  ...
  app/
    ...
    components/
      ...
      dd-format.ts                  # .dd variant format (DDDocument, DDNode)
      dd-parser.ts                  # HTML/CSS <-> .dd bidirectional parser
      dd-project.ts                 # .dd project file schema, validator, migrator
      dd-project-store.ts           # IndexedDB persistence for .dd project files
...
### 2.1 DesignDead Engine

**File:** `designdead-engine.tsx`  
**Export:** `<DesignDead />` component  

**How it works:**
- Creates a portal container `#designdead-portal` on `document.body`
...
- Auto-saves `.dd` project file to IndexedDB on state changes (debounced 500ms)
- Loads existing `.dd` project file from IndexedDB on mount
...
- **Element map** — A bidirectional map (`WeakMap<Element, string>` + `Map<string, Element>`) linking DOM elements to DesignDead IDs.
...
| `getElementById(id)` | Get DOM element by DesignDead ID |
...
- **Fork button** (green) — calls `onForkElementRequest` with the element's DesignDead ID
...
- During pan/zoom/drag, a global `<style>` is injected: `[data-designdead] iframe { pointer-events: none !important; }`
- Removed when interaction ends
- Prevents iframes from stealing mouse events

**Fork flow:**
1. `handleForkPage(viewportWidth)` → calls `capturePageSnapshot()`, creates `VariantData`, dispatches `ADD_VARIANT`, saves to IndexedDB
...
- DesignDead/ReactFlow CSS filtered out (rules containing `[data-designdead`, `.react-flow`, `--xy-`, `--dd-`)
...
- Dispatches `ADD_FEEDBACK` to store
...
- MCP flow instruction: "Call `designdead_push_changes` with exact variantId"
...
1. **IDE** — Cards for each supported IDE:
   - Claude Code (MCP setup: `claude mcp add designdead -- npx @zerosdesign/design-dead mcp`)
...
**Left section:**
- DesignDead logo + "designdead" label
...
**Right section:**
- **.dd file actions:** Download .dd (export), Import (upload), Sync to IDE (refresh)
...
| `ddProject` | `DDProject` | Project metadata |
| `ddProjectFile` | `DDProjectFile \| null` | Full .dd project file |
...
### 4.1 Variant .dd Format

**Files:** `dd-format.ts`, `dd-parser.ts`

A structured JSON representation of individual UI variants, inspired by Pencil.dev's `.pen` format.

**DDDocument structure:**
```typescript
{
  version: "0.1.0",
  name: string,
  source: { type: "page" | "component", selector?, route?, elementId? },
  variables?: Record<string, DDVariable>,    // Design tokens
  breakpoints?: DDBreakpoints,               // desktop/tablet/mobile
  tree: DDNode[]                             // Node tree
}
```
...
```typescript
{
  ...
  styles?: DDStyles,                         // Base CSS properties
  responsive?: { [breakpoint]: Partial<DDStyles> },  // Responsive overrides
  children?: DDNode[]
}
```

**Bidirectional parsers:**
- `htmlToDDDocument(html, css, name, source)` → parses HTML/CSS into DDDocument (uses temporary iframe for computed styles)
- `ddDocumentToHtml(doc)` → renders DDDocument to `{ html, css }` strings (generates `@media` queries from responsive overrides)

**Helper functions:** `findNodeById`, `updateNodeById`, `deleteNodeById`, `insertNode`, `countDDNodes`, `createDDDocument`

### 4.2 Project .dd File

**Files:** `dd-project.ts`, `dd-project-store.ts`

A single JSON file representing the **entire DesignDead project** — all variants, feedback, annotations, breakpoints, variables, pages, and history.
...
```json
{
  ...
  "integrity": { "hash": "sha256-...", "generator": "design-dead@0.0.5" }
}
```
...
| Function | Description |
|----------|-------------|
| `validateDDProjectFile(input)` | Zod validation, returns `{valid, data}` or `{valid, errors}` |
...
| `stateToProjectFile(project, variants, feedback, fileMappings, route, existing?)` | Converts runtime state → .dd |
| `projectFileToState(file)` | Converts .dd → runtime state |
| `serializeProjectFile(file)` | JSON.stringify with 2-space indent |
| `parseProjectFile(json)` | Parse + validate + migrate |

**IndexedDB persistence:**
- Database: `designdead-projects` (separate from variant DB)
- Stores: `dd-projects` (keyed by `project.id`), `dd-sync-meta` (sync tracking)
- Sync metadata: `lastSyncedRevision`, `lastSyncedAt`, `dirty` flag, `filePath`

**Import/Export:**
- `downloadProjectFile(file)` — triggers browser download as `{name}.dd`
...
- `pushProjectToIDE(file, port)` — POST to `http://127.0.0.1:{port}/api/dd-project`
...
| `designdead_update_node` | Update node in variant's .dd tree |
| `designdead_add_node` | Insert node into variant's .dd tree |
| `designdead_delete_node` | Remove node from variant's .dd tree |
| `designdead_set_variable` | Set design variable/token |
| `designdead_get_variables` | List design variables |
| `designdead_get_project_file` | Get full .dd project file as JSON |
...
### Database 1: `designdead-db` (variant-db.ts)
| Store | Key | Contents |
|-------|-----|----------|
...
### Database 2: `designdead-projects` (dd-project-store.ts)
| Store | Key | Contents |
|-------|-----|----------|
| `dd-projects` | `project.id` | Full `DDProjectFile` objects |
| `dd-sync-meta` | `projectId` | Sync tracking metadata |
...
- `testing/vite.config.ts` — Vite config that aliases `@zerosdesign/design-dead` → `../src/index.ts`
- `testing/index.html` — HTML entry point
- `testing/main.tsx` — Renders `HomePage` + `<DesignDead defaultOpen />`
...
### Core Infrastructure
- [x] `<DesignDead />` component with portal, keyboard shortcut, FAB toggle
...
- [x] srcdoc with viewport meta, import rule separation, DesignDead CSS filtering
...
### .dd Format
- [x] DDDocument/DDNode/DDStyles types with responsive breakpoints
- [x] Variables (design tokens) support
- [x] Bidirectional HTML/CSS ↔ .dd parser
- [x] .dd project file schema (Zod validated, SHA-256 integrity)
- [x] Revision-based conflict detection
- [x] Migration pipeline (v1 → v2 → ...)
- [x] State ↔ .dd project file bidirectional converter
- [x] IndexedDB persistence for project files
- [x] Export (download .dd) / Import (file picker)
...
- [x] .dd project file endpoints (read/write/save)
...
7. **.dd file is raw JSON in IDE** — Without a VS Code extension, opening `.dd` files shows raw JSON instead of a visual canvas.
...
- Custom `TextEditorProvider` for `*.dd` files
- Webview canvas renderer (reuse DesignDead React components)
- File-based sync: extension watches `.dd` file changes → webview updates
- Bidirectional: webview edits write back to `.dd` file
- "Open as Text" fallback
- Extension marketplace publishing
- Cursor/Windsurf compatibility (VS Code extension model)

### Phase B: File Watcher Sync
- IDE extension watches workspace `.dd` file for external changes
- Browser detects when IDE updates the `.dd` file (via bridge polling)
...
- Unit tests for .dd parser roundtrip
...
*Generator: design-dead documentation*
