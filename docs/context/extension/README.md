...
> **Doc label (PR 4):** **Superseded** — the Zeros **VS Code extension** is frozen; this entire document is **historical**. Native shell + engine: [../../Zeros-Structure/03-Mac-App-Architecture.md](../../Zeros-Structure/03-Mac-App-Architecture.md), index: [../../Zeros-Structure/12-Doc-Index-And-Labels.md](../../Zeros-Structure/12-Doc-Index-And-Labels.md).
>
...
> into the engine (`src/engine/`), which now runs as a **Node sidecar**
> in the **Electron** Mac app. The extension code under `extensions/vscode/`
> remains
...
3. **Status bar** -- creates a right-aligned status bar item with command `Zeros.sendToAgent`
...
2. Writes context to `.zeros/ai-request.md`
...
5. **Atomic write**: writes to a `.zeros-tmp` file first, then renames it over the original
...
A webview view provider registered on the Activity Bar at `Zeros.dashboardView`.
...
- **Send Feedback to Agent** -- executes `Zeros.sendToAgent` command (shows pending count)
- **Copy Feedback** -- executes `Zeros.copyFeedback` command
...
Registered as `Zeros.ocEditor` with `priority: "default"` for `*.0c` files. Instead of showing raw JSON, displays a project dashboard.
...
Reads `.zeros/.port` file. If a valid port is found, returns `{ running: true, url: "http://localhost:{port}" }`.
...
3. Creates a VS Code terminal named "Zeros Dev Server"
4. Runs the command in the terminal
5. Waits up to **15 seconds** for `.zeros/.port` to appear (polls every 500ms)
...
1. Writes markdown to `.zeros/feedback.md` in the workspace
...
1. Opens chat via `workbench.action.chat.open` with a prompt referencing `.zeros/feedback.md`
...
1. Creates a VS Code terminal named "Zeros -> Claude"
2. Sends command: `claude "Read .zeros/feedback.md and fix all the visual feedback issues..."`
...
6. **Pending feedback:** contents of `.zeros/feedback.md` (if exists)
7. **Instructions:**
...
|---------|-------|--------|
| `Zeros.sendToAgent` | Send Feedback to Agent | Fetch pending feedback, dispatch to detected agent |
| `Zeros.copyFeedback` | Copy Feedback to Clipboard | Format and copy pending feedback as markdown |
| `Zeros.showFeedback` | Show Pending Feedback | Open formatted feedback in a new editor tab |
| `Zeros.openBrowser` | Open in Browser | Read port from `.zeros/.port`, open in external browser |
| `Zeros.startDevServer` | Start Dev Server | Run `devServer.start()` to launch Vite in a terminal |
...
- **Offline:** `$(circle-slash) Zeros` -- bridge offline, no background
- **Pending:** `$(comment-discussion) Zeros: {count}` -- warning background, click to send
- **Connected:** `$(check) Zeros` -- connected to agent, no background
...
| `mcp-server.ts` | `ZerosMcpServer` -- 5 MCP tools for external AI agents |
...
| `messages.ts` | Bridge message types (mirror of `src/zeros/bridge/messages.ts`) |
