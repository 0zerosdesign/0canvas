# 0canvas (DesignDead) — Complete Project Analysis & Documentation

> **Package:** `@zerosdesign/design-dead`
> **Version:** 0.0.5
> **License:** MIT
...
**DesignDead** (codenamed `0canvas`) is a **visual feedback engine for AI-powered development**. It is a browser overlay tool that allows designers and developers to inspect live DOM elements, edit CSS styles in real-time, create design variants by "forking" pages or components, annotate designs, and send structured feedback/instructions to AI coding agents (like Claude Code, Cursor, Windsurf, VS Code Copilot, etc.) — all without leaving the browser.
...
DesignDead enables a workflow where:

1. **A developer runs their web app** (any framework: React, Vue, Svelte, Angular, etc.)
2. **They add `<DesignDead />` to their app** — a single React component
...
1. **Portal Rendering:** The `<DesignDead />` component creates a portal on `document.body` with an extremely high z-index (`2147483640`), ensuring it floats above everything in the consumer's app.

2. **Iframe Isolation:** The consumer's app is loaded inside an `<iframe name="designdead-preview">`. The DesignDead component detects when running inside this iframe and returns `null` (iframe guard), preventing recursive rendering.
...
├── package.json                  # npm package config (@zerosdesign/design-dead)
...
├── designdead.deps.json          # Dependency metadata
...
│   │   │
│   │   └── components/
│   │       ├── designdead-engine.tsx      # Main <DesignDead /> component
...
│   │       ├── dd-format.ts              # .dd variant format types + helpers
│   │       ├── dd-parser.ts              # HTML/CSS ↔ .dd bidirectional parser
│   │       ├── dd-project.ts             # .dd project file schema (Zod)
│   │       ├── dd-project-store.ts       # IndexedDB persistence + import/export
...
└── vscode-extension/
    ...
    ├── tsup.config.ts                    # Extension host build
    ├── vite.webview.config.ts            # Webview build
    ├── src/
    │   ├── extension.ts                  # Extension activation
    │   ├── providers/
    │   │   └── DDCustomEditorProvider.ts  # Custom editor for .dd files
    │   ├── shared/
    │   │   ├── ddFile.ts                 # .dd file parser/serializer
...
### 5.1 DesignDead Engine (`designdead-engine.tsx`)
- **Role:** Main entry component that consumers import
- **What it does:** Creates a portal overlay, handles keyboard shortcut (`Ctrl+Shift+D`), manages open/close state, renders the full workspace, polls MCP bridge every 2 seconds, auto-saves `.dd` project file to IndexedDB (500ms debounce), loads existing project on mount
...
- **What it does:** Logo, project switcher, MCP status badge, route switcher, panel toggle buttons (with count badges), .dd file actions (download/import/sync), viewport presets, Cmd+K button

### 5.16 .dd Format System (`dd-format.ts`, `dd-parser.ts`)
- **Role:** Design-as-code variant format
- **What it does:** Structured JSON representation of UI variants (DDDocument, DDNode, DDStyles), responsive breakpoints per node, design variables (tokens), bidirectional HTML/CSS ↔ .dd parser, tree CRUD helpers (find, update, delete, insert, count)

### 5.17 .dd Project File (`dd-project.ts`, `dd-project-store.ts`)
- **Role:** Whole-project persistence format
- **What it does:** Single `.dd` JSON file containing all project data (metadata, workspace config, breakpoints, variables, pages, variants, annotations, feedback, history), Zod schema validation, SHA-256 integrity hashing, migration pipeline, state ↔ .dd bidirectional conversion, IndexedDB persistence, file export/import, auto-save (500ms debounce), push/pull to IDE via MCP bridge
...
- **What it does:** Node.js CLI binary that runs an HTTP server (port 24192) and MCP server (stdio transport), 17 registered MCP tools for reading feedback, getting/modifying variants, pushing changes, managing .dd trees, design variables, and project files

### 5.20 VS Code Extension (`vscode-extension/`)
- **Role:** Visual editor for `.dd` files inside VS Code
- **What it does:** Custom text editor provider for `*.dd` files, webview-based canvas renderer with variant cards, bidirectional sync (edits in webview write back to file), "Open as Text" fallback command, patch-based edit protocol with conflict detection
...
|----------|---------|--------|
| **Core** | `<DesignDead />` React component with portal overlay | Done |
...
| **.dd Format** | DDDocument/DDNode/DDStyles types | Done |
| **.dd Format** | Design variables (tokens) | Done |
| **.dd Format** | Responsive breakpoints per node | Done |
| **.dd Format** | Bidirectional HTML/CSS ↔ .dd parser | Done |
| **.dd Project** | Zod schema validation | Done |
| **.dd Project** | SHA-256 integrity hash | Done |
| **.dd Project** | Migration pipeline | Done |
| **.dd Project** | State ↔ .dd conversion | Done |
| **.dd Project** | IndexedDB persistence | Done |
| **.dd Project** | File export/import | Done |
| **.dd Project** | Auto-save (500ms debounce) | Done |
...
| **MCP** | .dd project file endpoints | Done |
| **VS Code** | Custom editor for .dd files | Done |
| **VS Code** | Webview canvas renderer | Done |
| **VS Code** | Bidirectional file sync | Done |
| **VS Code** | Patch-based edit protocol | Done |
...
| **Project** | `ddProject`, `ddProjectFile` |
...
- **DDDocument:** version, name, source info, variables, breakpoints, node tree
- **DDNode:** tag, class, styles, responsive overrides, children, text content
- **DDStyles:** 40+ CSS properties with design token references (`$variable.name`)
- **Bidirectional parsing:** `htmlToDDDocument()` ↔ `ddDocumentToHtml()`

### .dd Project File
A single JSON file representing the entire DesignDead project:
...
|----------|-------|-----|---------|
| `designdead-db` | `variants` | `id` | Individual VariantData objects |
| `designdead-db` | `waitlist` | `id` | Individual FeedbackItem objects |
| `designdead-db` | `projects` | `id` | StoredProject (project + variants + feedback) |
| `designdead-projects` | `dd-projects` | `project.id` | Full DDProjectFile objects |
| `designdead-projects` | `dd-sync-meta` | `projectId` | Sync tracking metadata |
...
- **Port:** 24192 (configurable via `DESIGNDEAD_PORT` env var)
...
| POST | `/api/dd-project` | Save .dd project file |
| GET | `/api/dd-project` | Get .dd project file |
| POST | `/api/dd-project/write` | Write .dd file to workspace filesystem |
| GET | `/api/dd-project/read` | Read .dd file from workspace filesystem |

### MCP Tools (17 registered)

| Tool | Description |
|------|-------------|
| `designdead_get_pending` | List pending feedback items |
| `designdead_get_variant` | Get variant HTML/CSS/metadata |
| `designdead_resolve_feedback` | Mark feedback as resolved |
| `designdead_push_changes` | Push modified HTML/CSS to variant preview |
| `designdead_list_variants` | List all variants |
| `designdead_get_project` | Get project info |
| `designdead_watch` | Long-poll for new feedback |
| `designdead_get_variant_tree` | Get .dd JSON tree for variant |
| `designdead_update_node` | Update node in .dd tree |
| `designdead_add_node` | Insert node into .dd tree |
| `designdead_delete_node` | Remove node from .dd tree |
| `designdead_set_variable` | Set design variable/token |
| `designdead_get_variables` | List design variables |
| `designdead_get_project_file` | Get full .dd project file |
| `designdead_save_project_file` | Save .dd project file (with revision conflict detection) |
| `designdead_get_project_meta` | Get project summary metadata |
| `designdead_write_project_to_workspace` | Write .dd file to workspace path |
...
### IDE Setup Examples
- **Claude Code:** `claude mcp add designdead -- npx @zerosdesign/design-dead mcp`
...
### Overview
A separate sub-project inside `vscode-extension/` that provides a visual editor for `.dd` files directly in VS Code.

### Components
- **Extension Host (`extension.ts`):** Registers `DDCustomEditorProvider` for `*.dd` files, registers "Open as Text" command
- **Custom Editor Provider (`DDCustomEditorProvider.ts`):** Implements `CustomTextEditorProvider`, manages webview lifecycle, handles bidirectional sync between `.dd` file content and webview canvas
- **Webview App:** React-based canvas renderer with variant cards, uses `@xyflow/react`, communicates with host via `postMessage` protocol

### Protocol Messages
- **Host → Webview:** `init` (initial doc), `documentUpdated` (external changes), `ack` (patch success), `reject` (patch failure), `error` (validation errors)
- **Webview → Host:** `ready` (webview loaded), `applyPatch` (edit request), `openAsText` (switch to text editor)

### Status
The VS Code extension is **implemented** with core functionality (custom editor, bidirectional sync, patch protocol). It is not yet published to the VS Code marketplace.
...
| **VS Code Extension** | VS Code API (CustomTextEditorProvider) |
...
- .dd format with bidirectional parsing
- .dd project file with IndexedDB persistence
- VS Code extension (custom editor)
...
- VS Code extension not published to marketplace
...
7. **.dd file is raw JSON in IDE** — Without the VS Code extension, `.dd` files show raw JSON.

8. **No image/asset handling** — Variants don't capture or store referenced images/assets.
...
|-------|-------|----------|
| **A** | VS Code Extension (marketplace publish, file watching) | HIGH |
| **B** | File Watcher Sync (IDE ↔ browser bidirectional .dd sync) | HIGH |
